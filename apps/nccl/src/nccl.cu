...
static ncclResult_t ncclBroadcastFallback(const void* sendbuff, void* recvbuff, size_t sendcount,
...
NCCL_API ncclResult_t ncclBroadcast(const void* sendbuff, void* recvbuff, size_t sendcount, ncclDataType_t datatype, int root,
  ...
  if (plan == nullptr) return ncclBroadcastFallback(sendbuff, recvbuff, sendcount, datatype, root, comm, stream);
