...
/// Unique ID for bootstrap initialization.
...
/// Base class for bootstrap implementations.
class Bootstrap {
 public:
  /// Constructor.
  Bootstrap(){};
  ...
  /// the call order, corresponding to the order of recv() calls on the receiving side. When
  /// the execution order of multiple send()s or recv()s between two ranks is unknown, use
...
/// TCP-based bootstrap implementation.
...
/// Bitset for transport flags.
using TransportFlagsBase = std::bitset<TransportFlagsSize>;
}  // namespace detail

/// Transport flags storage and manipulation.
...
/// Represents a block of memory registered to a Context.
/// RegisteredMemory does not own the memory it points to, but provides a way to transfer metadata about the memory
/// to other processes, allowing them to access the memory block.
