...
class Host2DeviceSemaphore {
 private:
  ...
  detail::UniqueGpuPtr<uint64_t> expectedInboundId_;
  std::unique_ptr<uint64_t> outboundId_;
...
class Host2HostSemaphore {
 private:
  Semaphore semaphore_;
  std::unique_ptr<uint64_t> expectedInboundId_;
  std::unique_ptr<uint64_t> outboundId_;
...
class MemoryDevice2DeviceSemaphore {
 private:
  Semaphore semaphore_;
  detail::UniqueGpuPtr<uint64_t> expectedInboundId_;
  detail::UniqueGpuPtr<uint64_t> outboundId_;
