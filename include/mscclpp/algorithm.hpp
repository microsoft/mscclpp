...
class Algorithm {
 public:
  ...
  /// @param contextKey The context key for caching algorithm contexts.
  ...
  virtual CommResult execute(std::shared_ptr<Communicator> comm, const void* input, void* output, size_t inputSize,
                             ...
                             bool symmetricMemory = false, int32_t contextKey = -1) = 0;

  /// Reset the algorithm state, clearing any cached contexts.
  /// @param contextKey If specified, only reset the context associated with this key.
  virtual void reset(int32_t contextKey = -1) = 0;
};
...
///
/// The context key can be either:
/// - A buffer-based key (baseSendBuff, baseRecvBuff, etc.) generated by ContextKeyGenFunc, or
/// - A simple int32_t key provided directly via the contextKey parameter of execute().
struct AlgorithmCtxKey {
  enum class Kind { Buffer, Custom };
  ...
  /// Construct a buffer-based key (backward-compatible).
  AlgorithmCtxKey(void* baseSendBuff, void* baseRecvBuff, size_t baseSendSize, size_t baseRecvSize, int tag)
      : kind(Kind::Buffer), buffer{baseSendBuff, baseRecvBuff, baseSendSize, baseRecvSize, tag} {}

  /// Construct a user-provided custom integer key.
  explicit AlgorithmCtxKey(int32_t key) : kind(Kind::Custom), customKey(key) {}

  AlgorithmCtxKey() : kind(Kind::Custom), customKey(-1) {}

  bool operator==(const AlgorithmCtxKey& other) const {
    if (kind != other.kind) return false;
    if (kind == Kind::Custom) return customKey == other.customKey;
    return buffer.baseSendBuff == other.buffer.baseSendBuff && buffer.baseRecvBuff == other.buffer.baseRecvBuff &&
           buffer.baseSendSize == other.buffer.baseSendSize && buffer.baseRecvSize == other.buffer.baseRecvSize &&
           buffer.tag == other.buffer.tag;
  }
};
...
struct hash<mscclpp::AlgorithmCtxKey> {
  std::size_t operator()(const mscclpp::AlgorithmCtxKey& key) const {
    std::size_t seed = 42;
    mscclpp::detail::hashCombine(seed, static_cast<int>(key.kind));
    if (key.kind == mscclpp::AlgorithmCtxKey::Kind::Custom) {
      mscclpp::detail::hashCombine(seed, key.customKey);
    } else {
      mscclpp::detail::hashCombine(seed, key.buffer.baseSendBuff);
      mscclpp::detail::hashCombine(seed, key.buffer.baseRecvBuff);
      mscclpp::detail::hashCombine(seed, key.buffer.baseSendSize);
      mscclpp::detail::hashCombine(seed, key.buffer.baseRecvSize);
      mscclpp::detail::hashCombine(seed, key.buffer.tag);
    }
    return seed;
  }
};
...
class NativeAlgorithm : public Algorithm {
 public:
  ...
  CommResult execute(std::shared_ptr<Communicator> comm, const void* input, void* output, size_t inputSize,
                     size_t outputSize, DataType dtype, ReduceOp op, cudaStream_t stream,
                     std::shared_ptr<Executor> executor, int nBlocks = 0, int nThreadsPerBlock = 0,
                     const std::unordered_map<std::string, uintptr_t>& extras = {}, bool symmetricMemory = false,
                     int32_t contextKey = -1) override;
  ...
  void reset(int32_t contextKey = -1) override;
