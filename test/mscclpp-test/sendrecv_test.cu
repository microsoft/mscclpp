...
__constant__ mscclpp::channel::sm::SmChannel constSmChans[2];
...
__global__ void kernel(int rank, size_t dataSize, size_t dataPerBlock) {
  ...
  mscclpp::channel::sm::SmChannel sendConn = constSmChans[0];
  mscclpp::channel::sm::SmChannel recvConn = constSmChans[1];
...
void SendRecvTestEngine::setupConnections() {
  ...
  cudaMemcpyToSymbol(constSmChans, smChannels.data(), sizeof(mscclpp::channel::sm::SmChannel) * smChannels.size());
}
