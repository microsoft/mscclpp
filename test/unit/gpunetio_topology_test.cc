...
// CPU-only tests of the production selector: no CUDA, MPI, verbs, or sysfs.
#include <algorithm>
#include <iostream>
#include <stdexcept>
#include <string>
...
int main() {
  try {
    // Four GPUs, two CPU NUMA domains, two equally local HCAs per domain.
    // Both GPUs on each side must retain the FULL pair, not get one NIC each.
    ...
    int checked = 0;
    do {
      // Reordered/subset GPU visitation models remapped CUDA ordinals and MPI
      // ranks. The physical GPU locality alone must determine its HCA set.
      for (int gpu : {3, 0, 2, 1, 0, 3}) {
        expect(selectClosestHcas(gpuPaths[gpu], gpu / 2, hcas), expected[gpu]);
        ++checked;
      }
    } while (std::next_permutation(hcas.begin(), hcas.end(),
                                   [](const HcaTopology& a, const HcaTopology& b) { return a.name < b.name; }));

    // PCI affinity refines NUMA affinity; do not blindly select every HCA on
    // the same NUMA node if one is farther from the GPU's PCI subtree.
    ...
    expect(selectClosestHcas(gpu, 0, {}), {});  // discovery reports no-active-HCA before calling the selector
