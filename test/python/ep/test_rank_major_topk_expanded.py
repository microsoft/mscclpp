...
"""Real-GPU regression for the LOW_LATENCY expanded-layout alias API.
...
Use --capacity 133 for combine reuse, or 257 for both dispatch and combine
shared-tile reuse, with --topk 1/8/9 and --expect-domain-size. Capacity 1/2
also exercises empty source/stripe paths; these are correctness, not timing runs.
...
sentinel/zero, even after shrinking. Expanded expert storage ALIASES dispatch
storage. Snapshot dispatch before overwriting it with unweighted expert results;
combine applies original weights once in source FP32, then converts to BF16.

Expert results are input copies plus exact integer expert/slot tags to expose
duplicate-slot aliasing. Inactive and valid zero-weight expert rows are NaN.
Identity-expert cases distinguish source FP32 from per-destination BF16 rounding
and enforce original top-k summation order under cancellation. Nine paired
operations, including route/shape changes, share one CUDA graph; snapshots of
every pair are checked outside capture after every replay. Adapted from a40b27e.
"""
...
def _make_sample(name, rank, ranks, capacity, topk, hidden):
    ...
    if name == "shrunk_mixed":
        ...
        tokens = max(1, capacity - 2)
    elif name == "empty_all":
        tokens = 0
    elif name in ("source_fp32", "topk_order"):
        tokens = 1
    ...
    for token in range(tokens):
        ...
        elif name == "topk_order":
            # ((2**24 + 1) - 2**24) == 0 in FP32; grouping equal-owner
            # slots 0 and 2 first incorrectly returns 1. K<3 is a smoke case.
            values = [1.0] * hidden
            route = [rank * LOCAL_EXPERTS, ((rank + ranks // 2) % ranks) * LOCAL_EXPERTS]
            route += [rank * LOCAL_EXPERTS] * max(0, topk - 2)
            route = route[:topk]
            w = ([float(1 << 24), 1.0, -float(1 << 24)] + [0.0] * max(0, topk - 3))[:topk]
        x.append(values)
        ids.append(route)
        weights.append(w)
    return Sample(name, x, ids, None if name == "implicit_unit" else weights, name not in ("source_fp32", "topk_order"))
...
def _execute_step(moe, expert_output, step):
    ...
    # Snapshot BEFORE expert writes: expanded expert storage aliases these tokens.
    ...
    if expert_output.data_ptr() != dispatch_out.tokens.data_ptr():
...
def _validate_step(torch, ep, step, expert_output, args, rank, ranks, label):
    ...
    for tensor, shape, dtype in (
        ...
        (dispatch_out.combine_input_buffer, (rows, args.hidden), torch.bfloat16),
    ...
    assert dispatch_out.combine_input_buffer.data_ptr() == expert_output.data_ptr(), prefix
    assert step.tokens.data_ptr() != expert_output.data_ptr(), prefix
...
def run(args):
    # Initialize MPI before GPU imports so later failure aborts the world rather
    # than leaving peers waiting in a collective/kernel.
    ...
    try:
        ...
        # Gather actual source inputs/routing/weights before any CUDA allocation.
        ...
        assert expected_combines[CASES.index("source_fp32")] == [[81.0 if args.topk == 1 else 16.25] * args.hidden]
        assert expected_combines[CASES.index("topk_order")] == [
            [float(1 << 24) if args.topk < 3 else 0.0] * args.hidden
        ]
        ...
        # rounding/order witnesses -> dense again, on the SAME communicator/storage.
        ...
        # Exactly one warm dispatch + expert writes + combine on capture stream.
        comm.Barrier()
...
class CpuReferenceTests(unittest.TestCase):
    ...
    def test_source_reduces_in_original_topk_order(self):
        for topk in (1, 2, 3, 8, 9):
            sample = _make_sample("topk_order", 0, 8, 8, topk, 8)
            self.assertEqual(_combine_reference(sample, 32), [[float(1 << 24) if topk < 3 else 0.0] * 8])
            if topk >= 3:
                self.assertEqual(sample.ids[0][0], sample.ids[0][2])
                self.assertNotEqual(sample.ids[0][0], sample.ids[0][1])
        self.assertEqual(_fp32(_fp32(float(1 << 24) - float(1 << 24)) + 1.0), 1.0)
    ...
    def test_full_capacity_single_slot_and_multi_token_blocks(self):
        # Dispatch has 128 payload blocks and switches to two warp groups per
        # block above 128 tokens. It needs 257 tokens to reuse a shared tile;
        # combine uses 128 workers and already reuses its tile at 133 tokens.
        for topk in (1, 8, 9):
            for capacity in (1, 2, 8, 133, 257):
    ...
    def test_tiny_capacity_fixtures_stay_within_allocation(self):
        for capacity in (1, 2):
            self.assertEqual(parse_args(["--capacity", str(capacity)]).capacity, capacity)
            for topk in (1, 8, 9):
                for name in CASES:
                    samples = [_make_sample(name, rank, 2, capacity, topk, 8) for rank in range(2)]
                    self.assertTrue(all(len(sample.x) <= capacity for sample in samples))
                    for destination in range(2):
                        result = _dispatch_reference(samples, destination, capacity, topk, 8, 8)
                        self.assertEqual(sum(result.counts), len(result.payloads))

    def test_execute_step_snapshots_before_alias_writes(self):
        # Control-flow receipt, not GPU emulation: specifically catches moving
        # the snapshot after in-place expert writes or restoring a self-copy.
        events = []

        class Tensor:
            def __init__(self, name, ptr):
                self.name, self.ptr = name, ptr

            def data_ptr(self):
                return self.ptr

            def copy_(self, other):
                events.append((self.name, "copy", other.name))

            def add_(self, bias):
                events.append((self.name, "add"))

            def masked_fill_(self, poison, value):
                events.append((self.name, "poison"))

        case = SimpleNamespace(x=None, ids=None, weights=None, bias=None, poison=None)
        output = SimpleNamespace(
            tokens=Tensor("dispatch", 1),
            topk_ids=Tensor("received_ids", 2),
            weights=Tensor("received_weights", 3),
            layout=SimpleNamespace(num_tokens_per_rank=Tensor("received_counts", 4)),
        )
        step = SimpleNamespace(case=case, combined=object())
        for index, name in enumerate(("tokens", "ids", "weights", "counts")):
            setattr(step, name, Tensor(name, 10 + index))
        moe = SimpleNamespace(
            dispatch=lambda *args, **kwargs: (output, object()),
            combine=lambda *args, **kwargs: events.append(("combine",)),
        )
        _execute_step(moe, Tensor("expert_alias", 1), step)
        self.assertEqual(
            events,
            [
                ("tokens", "copy", "dispatch"),
                ("ids", "copy", "received_ids"),
                ("weights", "copy", "received_weights"),
                ("counts", "copy", "received_counts"),
                ("expert_alias", "add"),
                ("expert_alias", "poison"),
                ("combine",),
            ],
