...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        "It stores one unweighted expert row per top-k slot: tokens/expert output [R*T*K,H], "
        "row=(sourceRank*T+sourceToken)*K+slot, flat int32 IDs and FP32 weights [R*T*K]. "
        "R=ranks, T=tokens per rank, K=top-k, H=hidden; invalid metadata uses sentinel E=num-experts "
        "(or the API-configured invalid ID), counts are valid expanded rows per source, not unique tokens. "
        "Combine skips invalid/out-of-range/zero-weight rows and applies each original weight once "
        "in a source-side FP32 sum across K, without legacy rank-major partial-sum rounding.",
    )
    p.add_argument(
        "--validate",
        action="store_true",
    ...
    if args.ep_layout == "rank_major_topk_expanded":
        ...
        if args.num_topk > 9:
            raise SystemExit("--ep-layout rank_major_topk_expanded requires --num-topk <= 9")
