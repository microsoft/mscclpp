...
Adapted from the a40b27e API tests, with the target's registered-buffer alias ABI.
...
class TopkExpandedPythonApiTests(unittest.TestCase):
    def setUp(self):
        importlib.import_module("numpy")  # Load the real utils dependency before isolated imports.
        ...
        def runtime(*args, **kwargs):
            ...
            if kwargs["output_layout"] == cpp.DispatchLayout.RANK_MAJOR_TOPK_EXPANDED:
                result.expert_output_buffer_ptr.return_value = result.output_tokens_buffer_ptr.return_value
            return result
    ...
    def test_expanded_buffers_and_original_weights_forwarded_once(self):
        for k, weighted, blocks, explicit in product((1, 3, 8, 9), (False, True), (4, 130), (False, True)):
            with self.subTest(k=k, weighted=weighted, blocks=blocks, explicit=explicit):
                ...
                self.assertIs(output.combine_input_buffer, expert)
                self.assertEqual(expert.data_ptr(), output.tokens.data_ptr())
                ...
                self.assertIsNot(output.topk_ids, ids)
    ...
    def test_invalid_configuration_rejected_before_runtime_creation(self):
        self.reject(
            ...
            + [dict(max_tokens_per_rank=n) for n in (0, -1, True, 4.0)]
    ...
    def test_combine_tensor_handle_and_context_guards(self):
        moe = self.make()
        output, handle = moe.dispatch(self.x, self.ids, self.weights)
        ...
        contexts += [dict(topk_ids=output.topk_ids), dict(weights=output.weights)]
    ...
    def test_legacy_parity_and_cross_layout_handles(self):
        ...
        for layout in (None, self.layout.RANK_MAJOR):
            with self.subTest(layout=layout):
                ...
                native = moe._backend._runtime.cpp_runtime
                self.assertIsNone(output.combine_input_buffer)
                ...
                if rank_major:
                    ...
                    expert = moe.get_expert_output_buffer()
                    self.assertNotEqual(expert.data_ptr(), output.tokens.data_ptr())
                else:
    ...
    def test_old_separate_buffer_native_abi_is_rejected(self):
        original = self.factory.side_effect

        def separate(*args, **kwargs):
            native = original(*args, **kwargs)
            native.expert_output_buffer_ptr.return_value = next(_POINTERS)
            return native

        self.factory.side_effect = separate
        with self.assertRaisesRegex(RuntimeError, "aliased registered"):
            self.make()

    def test_alias_views_are_accepted_by_pointer_not_object_identity(self):
        moe = self.make(invalid_token_expert_id=-99)
        expert_view = replace(moe.get_expert_output_buffer())
        output, handle = moe.dispatch(self.x, self.ids, self.weights, output_buffer=expert_view)
        self.assertIsNot(expert_view, output.tokens)
        self.assertEqual(expert_view.data_ptr(), output.tokens.data_ptr())
        moe.combine(expert_view, handle)
        native = moe._backend._runtime.cpp_runtime
        self.assertEqual(native.ll_dispatch.call_args.args[15], -99)
        self.assertEqual(native.ll_combine.call_args.args[0], output.tokens.data_ptr())

    def test_full_capacity_shrinking_empty_and_stable_alias(self):
        for k in (1, 8, 9):
            moe = self.make(topk=k, max_tokens_per_rank=133)
            expert = moe.get_expert_output_buffer()
            for tokens in (133, 1, 0, 132, 133):
                ids = _Tensor((tokens, k), "int64")
                weights = _Tensor((tokens, k), "float32")
                output, handle = moe.dispatch(_Tensor((tokens, 128)), ids, weights)
                self.assertEqual(output.tokens.shape, (2 * 133 * k, 128))
                self.assertEqual(output.tokens.data_ptr(), expert.data_ptr())
                self.assertIs(handle.combine_context.topk_ids, ids)
                self.assertIs(handle.combine_context.weights, weights)
                self.assertEqual(handle.combine_context.max_tokens_per_rank, 133)
                self.assertEqual(moe.combine(output.tokens, handle).shape, (tokens, 128))

    def test_public_exports_and_default_combine_buffer(self):
        ep = importlib.reload(sys.modules["mscclpp.ep"])
        for name in ("RankMajorTopkExpandedDispatchHandle", "RankMajorTopkExpandedCombineContext", "DispatchLayout"):
            self.assertIn(name, ep.__all__)
            self.assertIs(getattr(ep, name), getattr(self.api, name))
        output = ep.DispatchOutput(self.x, None, ep.DispatchLayoutInfo(self.layout.TOKEN_MAJOR))
        self.assertIsNone(output.combine_input_buffer)
        self.assertEqual(ep.MoEMode.LOW_LATENCY.name, "LOW_LATENCY")
