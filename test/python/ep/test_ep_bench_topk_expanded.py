...
"""CPU-only adapter/CLI tests: NumPy tensor doubles, no CUDA, MPI or EP library.

These exercise the real benchmark functions, not the native dispatch/combine
kernels. Run directly with Python (unittest); NumPy is a project dependency.
"""
...
class _Tensor(np.ndarray):
    """Small tensor double for the operations used by these benchmark paths."""
...
class _MoeStub:
    ...
    def dispatch(self, x, ids, weights, *, output_buffer):
        ...
        recv_weights = np.tile(np.where(local, weights, 0), (self.ranks, 1))
        ...
        if self.layout == "expanded":
            ...
            tokens[recv_ids == e] = np.nan  # inactive payloads must not need sanitizing
        elif self.layout == "expert":
            tokens = np.zeros(output_buffer.shape)
    ...
    def combine(self, expert_input, handle, *, out):
        ...
        if self.layout != "expert":
            assert expert_input is self.expert_output, "combine requires registered expert output"
        else:
            assert expert_input is self.dispatch_out.tokens
        self.combined_inputs.append(expert_input.copy())
        # Independent scalar identity-expert oracle for validation tests. This
        # does not simulate remote transport or assert native payload-read safety.
        result = np.zeros(handle.x.shape, dtype=np.float32)
        for t in range(handle.x.shape[0]):
            for k in range(handle.ids.shape[1]):
                expert, weight = handle.ids[t, k], handle.weights[t, k]
                if 0 <= expert < self.config["num_experts"] and weight != 0:
                    result[t] = result[t].astype(np.float64) + handle.x[t].astype(np.float64) * float(weight)
        out.copy_(result)
...
class TopkExpandedBenchTests(unittest.TestCase):
    ...
    def setup(self, *, layout="rank_major_topk_expanded", validate=False, graph=False):
        flags = ["--ep-layout", layout, "-t", "2", "-d", "2", "-e", "4", "-k", "3"]
        flags += ["--validate"] if validate else []
        flags += ["--cuda-graph"] if graph else []
        args = self.args(*flags)
        # Duplicate local IDs, an out-of-range ID, and a zero-weight NaN token.
        x = _Tensor([[128, 256], [np.nan, np.nan]], "bf16")
        ids = _Tensor([[0, 0, 4], [1, -7, 5]], np.int64)
        weights = _Tensor([[0.125, 0.375, 9], [0, 3, 4]])
    ...
    def test_direct_adapter_rejects_invalid_expanded_config(self):
        for name, value in (("dispatch_dtype", "fp8_e4m3"), ("combine_mode", "direct_send"), ("num_topk", 10)):
    ...
    def test_expanded_eager_and_validation_use_unweighted_registered_buffer(self):
        ops, moe = self.setup(validate=True)
        ops["combine"](ops["dispatch"]())
        self.assertEqual(moe.layout, "expanded")
        self.assertEqual(moe.expert_output.shape, (2 * 2 * 3, 2))
        ...
        for payload in moe.combined_inputs:
            np.testing.assert_equal(payload, moe.dispatch_out.tokens)

    def test_expanded_graph_uses_registered_buffer_in_one_paired_loop(self):
        ...
        self.assertEqual(moe.calls, ["dispatch", "combine"] * 4)  # one prime + three captured pairs
        for payload in moe.combined_inputs:
            np.testing.assert_equal(payload, moe.dispatch_out.tokens)
        dispatch, combine, graph = captured
        combine(dispatch())
        graph.replay.assert_called_once_with()
        self.assertEqual(len(moe.combined_inputs), 4)  # replay-combine is a no-op
    ...
    def test_reference_has_no_per_rank_bf16_rounding(self):
        # Per-rank rounding gives 256; source FP32 accumulation then BF16 gives 258.
        x = _Tensor([[128]], "bf16")
    ...
    def test_adapter_validation_uses_source_fp32_reference(self):
        ...
        # Larger scale makes legacy rounding differ by more than the tolerance 8.
