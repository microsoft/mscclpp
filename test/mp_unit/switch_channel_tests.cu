...
__constant__ mscclpp::DeviceMulticastPointerDeviceHandle gConstSwitchChan;

__global__ void kernelSwitchReduce() {
  float *mcPtr = reinterpret_cast<float*>(gConstSwitchChan.mcPtr);
  uint1 val;
  mscclpp::DeviceMulticastPointerDeviceHandle::multimemLoadReduce(val, mcPtr);
  mscclpp::DeviceMulticastPointerDeviceHandle::multimemStore(val, mcPtr);
}
