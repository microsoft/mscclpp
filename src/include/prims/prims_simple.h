...
class Primitives<
 ...
 public:
  ...
  __device__
  Primitives(int tid, int nthreads, int const *recvPeers, int const *sendPeers,
             void const *inputBuf, void *outputBuf,
             struct ncclDevChannelPeer *channel_peer, uint64_t redOpArg,
             uint32_t group = 0, struct ncclWorkElem *e = nullptr)
      : tid(tid), stepSize(65536)
  {
    ...
    this->nthreads = nthreads;
    this->nworkers =
        nthreads - (MaxSend > 0 && nthreads - WARP_SIZE >= 64 ? WARP_SIZE : 0);
    this->group = group & (uint16_t)0xFFFF;
    int connIndex = group >> 16;

    int nrecv = 0, nsend = 0;
    while (nrecv < MaxRecv && recvPeers[nrecv] != -1)
      nrecv++;
    while (nsend < MaxSend && sendPeers[nsend] != -1)
      nsend++;
    this->fan = Fan(nrecv, nsend);
