...
MSCCLPP_API_CPP RegisteredMemory Context::registerMemory(void* ptr, size_t size, TransportFlags transports) {
  return RegisteredMemory(
      std::make_shared<RegisteredMemory::Impl>(ptr, size, -1, transports, *pimpl));
}

MSCCLPP_API_CPP Endpoint createEndpoint(Transport transport, int ibMaxCqSize, int ibMaxCqPollNum, int ibMaxSendWr, int ibMaxWrPerSend) {
  return Endpoint(std::make_shared<Endpoint::Impl>(transport, ibMaxCqSize, ibMaxCqPollNum, ibMaxSendWr, ibMaxWrPerSend, *pimpl));
}
...
MSCCLPP_API_CPP std::shared_ptr<Connection> Context::connect(Endpoint localEndpoint, Endpoint remoteEndpoint) {
  ...
  if (localEndpoint.transport() == Transport::CudaIpc) {
    conn = std::make_shared<CudaIpcConnection>(localEndpoint, remoteEndpoint, *pimpl);
  } else if (AllIBTransports.has(localEndpoint.transport())) {
    conn = std::make_shared<IBConnection>(localEndpoint, remoteEndpoint, *pimpl);
  } else {
