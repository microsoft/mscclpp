...
MSCCLPP_API_CPP BufferHandle HostConnection::registerBuffer(void* data, uint64_t size)
{
  ...
  mscclppRegisterBufferForConnection(pimpl->comm->pimpl->comm, pimpl->conn->connId, data, size,
                                     reinterpret_cast<mscclppBufferHandle_t*>(&result));
  return result;
}
...
MSCCLPP_API_CPP void HostConnection::put(BufferHandle dst, uint64_t dstOffset, BufferHandle src, uint64_t srcOffset,
                                         uint64_t size)
{
