...
//                           cudaStream_t stream, uint32_t*, uint32_t*, uint32_t*, uint32_t, int nBlocks,
...
//                                                             channelInOffset, channelOutOffset, inputSize, rank,
//                                                             nRanksPerNode);
...
//   AllreduceFunc allreduce = dispatch<NvlsAdapter>(Algorithm::Op::SUM, dtype);
//   if (!allreduce) {
...
//                 inputSize, stream, nullptr, nullptr, nullptr, 0, numBlocksAndThreads.first, numBlocksAndThreads.second);
