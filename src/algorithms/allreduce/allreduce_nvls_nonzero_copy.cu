...
struct NvlsWithCopyAdapter {
  static cudaError_t call(const void* input, void* scratch, void* output, void* memoryChannels, void*,
                          ...
                          cudaStream_t stream, LL8Packet*, uint32_t, int nBlocks, int nThreadsPerBlock) {
...
CommResult AllreduceNvlsWithCopy::allreduceKernelFunc(const std::shared_ptr<AlgorithmCtx> ctx, const void* input,
                                                      void* output, size_t inputSize, DataType dtype,
                                                      cudaStream_t stream,
...
std::shared_ptr<AlgorithmCtx> AllreduceNvlsWithCopy::initAllreduceContext(std::shared_ptr<Communicator> comm,
