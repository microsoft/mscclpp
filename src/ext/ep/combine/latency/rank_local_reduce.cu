...
__global__ __launch_bounds__(detail::CombineNThreads, 1) void latencyRankLocalReduceKernel(
  ...
  detail::latencyBody<CombineMode::RANK_LOCAL_REDUCE, Hidden, DispatchType, ScaleBlockSize, Layout>(
      output, expertOutput, topkIndices, topkWeights, srcInfo, layoutRange, workload, combineRecvBuffer,
      dispatchRecvBuffer, context);
}

struct LatencyRankLocalReduceKernelSelector {
  template <int Hidden, DispatchDataType DispatchType, int ScaleBlockSize, DispatchLayout Layout>
  static auto get() {
    return latencyRankLocalReduceKernel<Hidden, DispatchType, ScaleBlockSize, Layout>;
  }
};
...
void latencyRankLocalReduce(void* output, const void* input, const int64_t* topkIdx, const float* topkWeights,
                            ...
                            cudaStream_t stream) {
  detail::combineAlgorithm<CombineMode::RANK_LOCAL_REDUCE, LatencyRankLocalReduceKernelSelector>(
