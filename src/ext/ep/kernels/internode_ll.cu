...
__device__ __forceinline__ uint64_t peer_ptr_of(uint64_t local_ptr, void* const* peer_rdma_bases, void* rdma_buffer_ptr,
                                                int peer_rank) {
...
__device__ __forceinline__ void port_channel_barrier_block(mscclpp::PortChannelDeviceHandle* port_channel_handles,
...
__device__ __forceinline__ void memory_channel_barrier_block(mscclpp::MemoryChannelDeviceHandle* memory_channel_handles,
                                                             int rank, int num_ranks) {
  const int tid = threadIdx.x;
  if (tid < num_ranks && tid != rank) {
...
__device__ __forceinline__ void ll_barrier_block(mscclpp::PortChannelDeviceHandle* port_channel_handles,
                                                 mscclpp::MemoryChannelDeviceHandle* memory_channel_handles, int rank,
                                                 int num_ranks) {
...
template <int kNumThreads, bool kIpcPath>
__launch_bounds__(kNumThreads, 1) __global__
    void clean_low_latency_buffer(int64_t* clean_0, int num_clean_int_0, int64_t* clean_1, int num_clean_int_1,
                                  mscclpp::PortChannelDeviceHandle* port_channel_handles,
                                  mscclpp::MemoryChannelDeviceHandle* memory_channel_handles, int rank, int num_ranks) {
...
__global__ __launch_bounds__(kNumWarpGroups* kNumWarpsPerGroup * 32, 1) void dispatch(
  ...
  const size_t num_bytes_per_msg =
      sizeof(int4) + (kUseFP8 ? (kHidden + num_scales * sizeof(float)) : (kHidden * sizeof(nv_bfloat16)));
...
#pragma unroll
          ...
          if (lane_id == 0 or lane_id == 16) rdma_x_scales[i * kNumElemsPerRead / 128] = scale_inv;
...
#pragma unroll 8
    for (int i = lane_id; i < num_tokens * num_topk; i += 32) {
      ...
      if (idx >= expert_begin_idx and idx < expert_end_idx) expert_count[idx - expert_begin_idx]++;
    }
...
LOW_LATENCY_DISPATCH_RECV:
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    const auto recv_x_scales =
        packed_recv_x_scales + local_expert_idx * num_ranks * num_max_dispatch_tokens_per_rank * num_scales;
...
void dispatch(void* packed_recv_x, float* packed_recv_x_scales, int* packed_recv_src_info,
  ...
  const auto num_sms =
      std::max<int>(num_sms_base, std::min<int>(device_num_sms, std::max<int>(num_tokens, num_sms_base)));
  ...
  if (responsible_expert_idx < num_experts) {
    ...
    if (sub_warp_id == 1 and lane_id == 0) {
      ...
      if (dst_rank != rank) {
        if constexpr (kIpcPath) {
          auto peer_flag =
              reinterpret_cast<int64_t*>(peer_ptr_of(reinterpret_cast<uint64_t>(rdma_recv_flag + global_expert_idx),
                                                     peer_rdma_bases, rdma_buffer_ptr, dst_rank));
...
#pragma unroll
      for (int j = 0; j < kNumElemsPerInt4; ++j) combined_bf16[j] = static_cast<nv_bfloat16>(combined_values[j]);
...
void combine(void* combined_x, void* rdma_recv_x, int64_t* rdma_recv_flag, void* rdma_send_x, const void* x,
  ...
  const auto num_sms =
      std::max<int>(num_sms_base, std::min<int>(device_num_sms, std::max<int>(num_combined_tokens, num_sms_base)));
