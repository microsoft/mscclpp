...
NB_MODULE(mscclpp_ep_cpp, m) {
  ...
  nb::class_<mscclpp::ep::high_throughput::Config>(m, "Config").def(nb::init<int>(), nb::arg("num_sms") = 20);

  nb::class_<mscclpp::ep::MoEHighThroughputRuntime>(m, "ExpertParallelRuntime")
      .def(nb::init<mscclpp::Communicator&, int64_t, const mscclpp::ep::high_throughput::Config&>(), nb::arg("comm"),
