...
struct Layout {
  ...
  void* expandedSyncFlags_ = nullptr;
  void* expandedSyncEpoch_ = nullptr;
  ...
  MSCCLPP_HOST_DEVICE_INLINE Layout(void* symmetricBuffer, int maxTokensPerRank, int hidden, int numRanks,
    ...
    const size_t gpuNetIoStagingBytes = configAlign<size_t>(
        static_cast<size_t>(topkExpanded ? maxTokensPerRank : GpuNetIoStagingSlots) * gpuNetIoSlotStride_,
        BufferAlignmentBytes);
    ...
    if (topkExpanded) totalBytes_ = expandedEpochOffset + BufferAlignmentBytes;
...
inline size_t symmetricBufferSize(int maxTokensPerRank, int hidden, int numRanks, int numExperts, int numTopk,
  ...
  const auto numBytes =
      Layout(nullptr, maxTokensPerRank, hidden, numRanks, numExperts, numTopk, rankMajor, topkExpanded).totalBytes_;
