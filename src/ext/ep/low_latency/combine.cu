      ...
      if (nTopk <= RankMajorTmaMaxNTopk) {
        if (blockIdx.x == 0) {
          ...
          recvRankMajorRemotePartialsTma<Hidden>(output, expertOutput, topkIndices, nTokens, nTopk, nExperts, nRanks,
                                                 maxTokensPerRank, workload.epoch_, transport, workspaceView,
                                                 sharedMemory);
        }
      } else {
...
void combine(void* output, const void* input, const int64_t* topkIdx, const float* topkWeights, const int* srcInfo,
  ...
  if (workload.outputLayout_ == DispatchLayout::RANK_MAJOR_TOPK_EXPANDED) {
    ...
    return topk_expanded::combine(output, input, topkIdx, topkWeights, workload, comm, workspace, numBlocks, stream);
  }
