...
void IBConnection::write(RegisteredMemory dst, uint64_t dstOffset, RegisteredMemory src, uint64_t srcOffset,
  ...
  NpKit::CollectCpuEvent(NPKIT_EVENT_CONN_CUDA_IB_WRITE_ENTRY, uint32_t(size), 0, *NpKit::GetCpuTimestamp(), 0);
#endif
  ...
  NpKit::CollectCpuEvent(NPKIT_EVENT_CONN_CUDA_IB_WRITE_EXIT, uint32_t(size), 0, *NpKit::GetCpuTimestamp(), 0);
#endif
}
