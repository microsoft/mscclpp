...
- `-DMSCCLPP_USE_GPUNETIO=ON`: Enable the optional GPUNetIO/GDAKI PortChannel backend (CUDA and IB required). Initialize `src/gpunetio/vendor` with `git submodule update --init --recursive` first; see [backend setup and usage](../src/gpunetio/PROVENANCE.md). Existing CPU-proxy channels remain the default.
