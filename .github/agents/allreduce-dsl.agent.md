...
- `python/mscclpp/language/tests/unit_tests/` — minimal DSL programs that each exercise **one primitive or fusion pattern**. Use these to learn correct usage and to copy idiomatic snippets:
  - `put/`, `get/`, `copy_test.py`, `flush/` — basic data movement.
  - `signal_wait/` (incl. `relaxed_*`) — synchronization patterns and `SyncType` usage.
  - `reduce/`, `reduce_send_test.py`, `read_reduce/`, `read_reduce_send_test.py` — reductions and reduce-fuse patterns.
  - `reduce_packet/`, `reduce_send_packet_test.py`, `reduce_copy_packet_test.py`, `reduce_copy_send_packet_test.py`, `put_packet_test.py`, `read_put_packet_test.py`, `copy_packet_test.py`, `unpack_packet_test.py` — LL/packet-protocol patterns.
  - `switch_reduce_test.py`, `switch_broadcast_test.py` — `SwitchChannel` (NVLS) primitives.
  - `tbg/` — `ThreadBlockGroup` patterns for heterogeneous TB workloads.
  - `pipeline_test.py` — `LoopIterationContext` pipelining.
  - `barrier_test.py`, `semaphore.py` — rank barriers and semaphores.
  - `*_fuse_test.py` files demonstrate which operations the DSL fuses (per `docs/dsl/concepts.md` § Operation Fusion).
  When in doubt about how to express a single primitive, **consult the matching unit test first** before borrowing from a full collective example.
...
- **Comments:** add a top-of-file block stating: hardware profile, message-size regime, algorithm family, channel topology, expected fusion, and the reference example(s) the design draws from (collective example + any unit_tests primitives). Inline comments should explain *why* (not *what*) for non-obvious synchronization / fusion choices.
...
| **Logical** (in generated code or design) | DSL compile produces invalid JSON, executor reports wrong reduction, correctness check fails | Stop. Surface the failure, propose a specific fix (1–3 options if more than one is reasonable), and ask the user which path to take. Do not silently rewrite the file. |
