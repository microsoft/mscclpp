...
0. **Interpreter check.** Confirm a real Python interpreter is on PATH before any compile/run step. On Windows, `python3.exe` is often a Microsoft Store launcher stub that prints an install prompt instead of executing scripts — detect this (e.g., `python3 -c "import sys; print(sys.executable)"` should print a real path, not a `WindowsApps` stub) and surface a clear error asking the user to install Python or use `python` / a venv interpreter.
