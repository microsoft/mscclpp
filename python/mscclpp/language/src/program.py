...
from mscclpp.language.json_generation.json_types import JsonProgram, JsonGpu, RemoteBuffer, JsonThreadblock
...
class MSCCLPPProgram:
    ...
    def generating_execution_plan(self):
        ...
        for rank in range(self.num_ranks):
                ...
                for op in operations[rank][tb]:
                    for remote_chunk in op.remote_chunks:
                        if op.channel_type == ChannelType.memory or op.channel_type == ChannelType.port:
                            ...
                            remote_buffer = RemoteBuffer(remote_chunk.rank, remote_chunk.buffer, op.channel_type)
            ...
            for tb in range(num_tb):
                tb_json = JsonThreadblock(tb, remote_buffer_internal_ids)
                for op in operations[rank][tb]:
