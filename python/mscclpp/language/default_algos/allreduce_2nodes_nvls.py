...
def allreduce_2nodes(spec: AlgoSpec, thread_block_group_size: int) -> CollectiveProgram:
    ...
    with CollectiveProgram.from_spec(spec) as prog:
        ...
        node_nvls_chan = [SwitchChannel(rank_list=[gpu for gpu in range(gpus_per_node)], buffer_type=BufferType.scratch),
                          SwitchChannel(rank_list=[gpu for gpu in range(gpus_per_node, 2 * gpus_per_node)], buffer_type=BufferType.scratch)]
        scratch_buffers = []
