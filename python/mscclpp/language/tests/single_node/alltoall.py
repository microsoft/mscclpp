    ...
    with MSCCLPPProgram(
        ...
        for gpus in range(gpu_size):
            ...
            for peer in range(gpu_size):
                ...
                if src_rank_id != peer:
                    tb = dst_rank_id if dst_rank_id < src_rank_id else dst_rank_id - 1
                    channels[dst_rank_id, src_rank_id].signal(tb=tb, relaxed=True, data_sync=SyncType.before)
            for peer in range(gpu_size):
                dst_rank_id = peer
                if src_rank_id != dst_rank_id:
                    tb = dst_rank_id if dst_rank_id < src_rank_id else dst_rank_id - 1
                    channels[dst_rank_id, src_rank_id].wait(tb=tb, relaxed=True)
...
""" alltoall_example("alltoall", 2, 1024, 0, 2**64 - 1) """