...
def allgather_example(name, gpus_per_node, num_threads_per_block, min_message_size, max_message_size):
    ...
    with MSCCLPPProgram(
        ...
        for gpu in range(gpu_size):
            ...
            rank.copy(scratch_buffer[gpu][gpu: gpu + 1],  input_buffer[gpu: gpu + 1], tb=gpu, to_packet=True)
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        ch.put_packet(scratch_buffer[dst_rank][src_rank : src_rank + 1], scratch_buffer[src_rank][src_rank: src_rank + 1], tb=peer, from_packet=True)
        ...
        for node in range(nodes):
            for gpu in range(gpus_per_node):
                ...
                ch.put_packet(scratch_buffer[dst_rank][src_rank : src_rank + 1], scratch_buffer[src_rank][src_rank: src_rank + 1], tb=gpu)
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        dst_rank = peer + gpus_per_node * node
                        ch = MemoryChannel(dst_rank, src_rank)
                        ch.put_packet(scratch_buffer[dst_rank][src_offset : src_offset + 1], scratch_buffer[src_rank][src_offset: src_offset + 1], tb=peer, from_packet=True)
        ...
        for gpu in range(gpu_size):
            for peer in range(1, gpu_size):
                ...
                rank = Rank(gpu)
                input_buffer = rank.get_output_buffer()
                rank.copy(input_buffer[dst_rank: dst_rank + 1], scratch_buffer[gpu][dst_rank: dst_rank + 1], tb=peer, from_packet=True)
...
allgather_example(args.name, args.gpus_per_node, args.num_threads_per_block, args.min_message_size, args.max_message_size)
