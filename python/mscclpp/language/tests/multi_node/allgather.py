    ...
    with MSCCLPPProgram(
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        memory_channels[(dst_rank_id, src_rank_id)].put(dst_buffer[src_rank_id: src_rank_id + 1], src_buffer[src_rank_id: src_rank_id + 1], tb=0)
        ...
        for node in range(nodes):
            for gpu in range(gpus_per_node):
                ...
                port_channels[src_rank_id].put_with_signal_and_flush(dst_buffer[src_rank_id : src_rank_id + 1], src_buffer[src_rank_id: src_rank_id + 1], tb=0)
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        dst_rank = Rank(dst_rank_id)
                        dst_buffer = dst_rank.get_output_buffer()
                        memory_channels[(dst_rank_id, src_rank_id)].put(dst_buffer[src_offset: src_offset + 1], src_buffer[src_offset: src_offset + 1], tb=0)
