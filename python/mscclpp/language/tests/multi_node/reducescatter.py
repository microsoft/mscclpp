    ...
    with MSCCLPPProgram(
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        memory_channels[(dst_rank_id, src_rank_id)].reduce(src_buffer[src_rank_id: src_rank_id + 1], [dst_buffer[src_rank_id: src_rank_id + 1]], tb=0)
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        dst_rank = Rank(dst_rank_id)
                        dst_buffer = dst_rank.get_input_buffer()
                        memory_channels[(dst_rank_id, src_rank_id)].reduce(src_buffer[next_src_rank_id: next_src_rank_id + 1], [dst_buffer[next_src_rank_id: next_src_rank_id + 1]], tb=0)
        ...
        for node in range(nodes):
            for gpu in range(gpus_per_node):
                ...
                port_channels[src_rank_id].put_with_signal(scratch_buffer[next_src_rank_id][0: 1], src_buffer[next_src_rank_id: next_src_rank_id + 1], tb=0)
        ...
        for node in range(nodes):
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        dst_rank_id = peer + gpus_per_node * node
                        memory_channels[(dst_rank_id, src_rank_id)].signal(tb=0, data_sync=SyncType.before, relaxed=True)
