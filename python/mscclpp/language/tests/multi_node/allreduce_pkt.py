...
def allreduce_example(
    program_name, gpus_per_node, thread_block_group_size, num_threads_per_block, 
    ...
    with CollectiveProgram(
            ...
            for local_gpu_id in range(gpus_per_node):
                ...
                for peer_gpu_id in range(gpus_per_node):
                    if peer_gpu_id != local_gpu_id:
                        ...
                        intra_node_memory_channels[(peer_rank_id, current_rank_id)] = MemoryChannel(peer_rank_id, current_rank_id)
        ...
        for node_id in range(num_nodes):
            for local_gpu_id in range(gpus_per_node):
                ...
                for peer_gpu_id in range(gpus_per_node):
                    ...
                    if peer_gpu_id != local_gpu_id:
                        intra_node_memory_channels[(peer_rank_id, current_rank_id)].put_packets(
                            scratch_buffers[peer_rank_id][local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu],
                ...
                # Intra Node Reduce
                other_gpu_data = [
                    scratch_buffers[current_rank_id][packets_per_gpu * gpu_idx: packets_per_gpu * gpu_idx + packets_per_gpu] 
                    for gpu_idx in range(gpus_per_node) if gpu_idx != local_gpu_id
                ]
                current_rank.reduce(
                ...
                current_rank.copy_packets(
                    scratch_buffers[current_rank_id][local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu], 
                    input_buffer[local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu], 
                    tb_group=thread_block_group
                )
                inter_node_offset = total_gpus
                inter_node_port_channels[current_rank_id].read_put_packets(
                    scratch_buffers[next_node_rank_id][inter_node_offset + local_gpu_id * packets_per_gpu: inter_node_offset + local_gpu_id * packets_per_gpu + packets_per_gpu],
                    scratch_buffers[current_rank_id][local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu],
                    tb=0
                )
                ...
                inter_node_data = [
                    scratch_buffers[current_rank_id][inter_node_offset + local_gpu_id * packets_per_gpu: inter_node_offset + local_gpu_id * packets_per_gpu + packets_per_gpu]
                ]
                current_rank.reduce(
                    input_buffer[local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu],
                    inter_node_data,
                ...
                for peer_gpu_id in range(gpus_per_node):
                    ...
                    if peer_gpu_id != local_gpu_id:
                        intra_node_memory_channels[(peer_rank_id, current_rank_id)].put_packets(
                            scratch_buffers[peer_rank_id][inter_node_offset + local_gpu_id * packets_per_gpu: inter_node_offset + local_gpu_id * packets_per_gpu + packets_per_gpu],
                            input_buffer[local_gpu_id * packets_per_gpu: local_gpu_id * packets_per_gpu + packets_per_gpu],
                            tb_group=thread_block_group
                        )
                ...
                for peer_gpu_id in range(gpus_per_node):
                    if peer_gpu_id != local_gpu_id:
                        current_rank.unpack_packets(
                            input_buffer[peer_gpu_id * packets_per_gpu: peer_gpu_id * packets_per_gpu + packets_per_gpu],
                            scratch_buffers[current_rank_id][inter_node_offset + peer_gpu_id * packets_per_gpu: inter_node_offset + peer_gpu_id * packets_per_gpu + packets_per_gpu],
                            tb_group=thread_block_group
                        )
