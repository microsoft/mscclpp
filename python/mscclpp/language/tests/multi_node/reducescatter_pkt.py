    ...
    with MSCCLPPProgram(
            ...
            for gpu in range(gpus_per_node):
                ...
                for peer in range(gpus_per_node):
                    if peer != gpu:
                        ...
                        memory_channels[(dst_rank_id, src_rank_id)].put_packet(scratch_buffer[dst_rank_id][dst_offset: dst_offset + 1], src_buffer[dst_rank_id: dst_rank_id + 1], tb=0)
                        memory_channels[(dst_rank_id, src_rank_id)].put_packet(scratch_buffer[dst_rank_id][r_dst_offset: r_dst_offset + 1], src_buffer[next_dst_rank_id: next_dst_rank_id + 1], tb=0)
        ...
        for node in range(nodes):
            for gpu in range(gpus_per_node):
                ...
                src_rank.copy(scratch_buffer[src_rank_id][gpu: gpu + 1], src_buffer[next_src_rank_id: next_src_rank_id + 1], tb=0, to_packet=True)
                port_channels[src_rank_id].put_packet(scratch_buffer[next_src_rank_id][gpu_size - 1: gpu_size], scratch_buffer[src_rank_id][gpu: gpu + 1], tb=0)
...
allreduce_example(args.name, args.gpus_per_node, args.num_threads_per_block, args.min_message_size, args.max_message_size)