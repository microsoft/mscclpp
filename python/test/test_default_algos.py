...
def test_allgather_multi_nodes_builds_serializable_program(
    ...
    program = allgather_multi_nodes(_allgather_spec(in_place=in_place), 1)
...
def test_allgather_multi_nodes_rejects_invalid_spec() -> None:
    ...
    with pytest.raises(ValueError, match="AllGather"):
        allgather_multi_nodes(invalid_collective, 1)
    with pytest.raises(ValueError, match="thread_block_group_size=1"):
        allgather_multi_nodes(spec, 2)
    ...
    with pytest.raises(ValueError, match="protocol='LL'"):
        allgather_multi_nodes(invalid_protocol, 1)
    ...
    with pytest.raises(ValueError, match="chunk_factor=1"):
        allgather_multi_nodes(invalid_chunk_factor, 1)
    ...
    with pytest.raises(ValueError, match="must match"):
        allgather_multi_nodes(inconsistent_in_place, 1)
