...
class Kernel:
    def __init__(self, ptx: bytes, args: dict):
        ...
        err, self._kernel = cuda.cuModuleGetFunction(
            self._module, kernel_name.encode())
    ...
    def launch_kernel(self, params: bytes, nblocks: int, nthreads: int, shared: int, stream):
        ...
        config = np.array([cuda.CU_LAUNCH_PARAM_BUFFER_POINTER, ctypes.addressof(buffer),
...
class KernelBase:
    def __init__(self, file, args):
        ...
        ptx = self._compile_cuda(os.path.join(
            self._current_file_dir, file), f"{args['KERNEL']}.ptx")
