...
class MyProxyService {
 ...
 public:
  MyProxyService(int my_rank, int nranks, int dataSize, std::vector<std::shared_ptr<mscclpp::Connection>> conns,
      std::vector<std::shared_ptr<mscclpp::RegisteredMemory>> allRegMem, std::vector<std::shared_ptr<mscclpp::Host2DeviceSemaphore>> semaphores)
      : my_rank_(my_rank), nranks_(nranks), dataSize_(dataSize), connections_(conns), allRegMem_(allRegMem), semaphores_(semaphores),
  ...
  mscclpp::ProxyHandlerResult handleTrigger(mscclpp::ProxyTrigger triggerRaw) {
    ...
    for (int r = 1; r < nranks_; ++r) {
      ...
      connections_[nghr]->write(*allRegMem_[nghr], my_rank_ * dataSizePerRank, *allRegMem_[my_rank_], my_rank_ * dataSizePerRank,
                                dataSizePerRank);
...
void init_mscclpp_proxy_test_module(nb::module_ &m) {
  nb::class_<MyProxyService>(m, "MyProxyService")
      .def(nb::init<int, int, int,
          std::vector<std::shared_ptr<mscclpp::Connection>>,
          std::vector<std::shared_ptr<mscclpp::RegisteredMemory>>,
          std::vector<std::shared_ptr<mscclpp::Host2DeviceSemaphore>>>(),
          nb::arg("rank"), nb::arg("nranks"), nb::arg("data_size"), nb::arg("conn_vec"), nb::arg("reg_mem_vec"), nb::arg("h2d_sem_vec"))
